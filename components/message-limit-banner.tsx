...
import { AlertTriangle, LogIn, UserPlus, CheckCircle, MessageSquare } from "lucide-react"
...
export function MessageLimitBanner() {
  const { messageCount, maxMessages, canSendMessage, hasReachedLimit, anonymousChat } = useAnonymousChat()
  const { user } = useAuth()
  const router = useRouter()
  const [showConversionSuccess, setShowConversionSuccess] = useState(false)

  // Show conversion success when user logs in with existing anonymous chat
  useEffect(() => {
    ...
    return (
      <Alert className="mb-4 border-green-200 bg-green-50 text-green-800 dark:border-green-800 dark:bg-green-950 dark:text-green-200">
        ...
        <AlertDescription className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <span>🎉 Welcome! Your anonymous chat has been saved to your account. You now have unlimited messaging!</span>
          <Button
            size="sm"
            variant="outline"
  ...
  if (canSendMessage && remainingMessages <= 2) {
    return (
      <Alert className="mb-4 border-yellow-200 bg-yellow-50 text-yellow-800 dark:border-yellow-800 dark:bg-yellow-950 dark:text-yellow-200">
        ...
        <AlertDescription>
          You have {remainingMessages} message{remainingMessages !== 1 ? "s" : ""} remaining in this anonymous session.{" "}
  ...
  // Show limit reached
  if (hasReachedLimit || !canSendMessage) {
    return (
      <Alert className="mb-4 border-red-200 bg-red-50 text-red-800 dark:border-red-800 dark:bg-red-950 dark:text-red-200">
        <MessageSquare className="h-4 w-4" />
        <AlertDescription className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <span>
            You've reached the {maxMessages} message limit for anonymous chats. Sign in to continue this conversation
            and get unlimited messaging.
          </span>
